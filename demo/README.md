...
| Permessi | Cambiando ruolo cambia davvero l'app. Il **collaboratore** non vede il pannello Admin né la classifica di rete, trova i **quattro pilastri in cima**, e nello scadenzario legge «I miei rinnovi» invece di quelli della rete. Il **leader** vede sé e i propri collaboratori, con un selettore per isolare la singola persona e il nome del titolare sulle righe altrui. L'**admin** vede tutto |
| Da fare adesso | In cima alla dashboard, **cosa richiede attenzione oggi**: scadute, richieste che aspettano il tuo sì, in scadenza entro quattordici giorni, prossimo appuntamento — in ordine di urgenza, ognuna che porta dove si risolve. Se non c'è niente lo dice, invece di riempire lo spazio: una schermata che trova sempre qualcosa da segnalare smette di essere letta |
| Di chi è questa riga | Nel CRM e nello scadenzario, chi guarda oltre sé stesso legge **«di Mario Rossi»** sulle righe altrui — e sulle proprie no, che sarebbe rumore su ogni riga. Serve soprattutto su «da approvare»: *«3 richieste»* senza i nomi si risolve aprendole una per una |
...
| Agente AI — memoria | **Cosa ricordo di te**: obiettivi, vincoli e preferenze che l'agente ha annotato parlandoti, così non devi ripeterli. Li cancelli uno per uno o tutti insieme. **Li vedi solo tu** — non il tuo leader, non l'amministratore: sono presi da conversazioni private, e nell'app è la RLS del database a garantirlo, non un pulsante nascosto |
| Agente AI — tetto | Sopra la chat c'è **quante domande hai fatto oggi**. L'agente chiama Claude a ogni risposta: senza limite, una persona che tiene premuto invio fa crescere il conto finché non arriva la fattura. Prova a superare le 40 e guarda cosa succede — il contatore si ferma e la domanda non parte. Avvisa all'80%, non a filo di limite: a quel punto non si fa più in tempo a cambiare comportamento |
...
| Funnel | La pagina pubblica vera è [`funnel/index.html`](../funnel/index.html) — un file solo, da caricare su qualunque hosting statico. Qui nella demo è simulata: **la pagina pubblica** che raccoglie contatti — l'unica porta dell'app senza login, e per questo la più esposta. C'è il modulo vero: compilalo e il contatto entra nel CRM col consenso registrato. Poi prova i cinque pulsanti **«Prova i filtri»**: campo civetta, invio in mezzo secondo, nessuna spunta, nessun recapito, limite orario superato — tutti respinti, e nessuno finisce fra i contatti. Il consenso è **una spunta per canale**: chi acconsente all'email non ha acconsentito a WhatsApp. Di ogni contatto resta **copia del testo accettato**, non un riferimento (visibile solo a leader e admin) |
...
| Rank | Rank a carte (2 → Asso) con **anello di avanzamento** e scomposizione dei punti. La classifica rispetta il perimetro vero: il **leader** vede sé e la propria squadra — non l'amministratore, che `can_read_member()` non gli darebbe mai — e il **collaboratore** legge «Dove ti trovi» invece di una classifica in cui sarebbe primo da solo |
...
| Community ♣ | Feedback della squadra. **«Elimina» compare solo sul tuo post** (cambia ruolo e guarda: si sposta), con conferma. Prima si poteva pubblicare una foto davanti a tutta la rete e non c'era modo di toglierla — il database l'ha sempre permesso, mancava il pulsante. Nell'app la foto viene cancellata insieme al post: il bucket è pubblico, e un file rimasto indietro resterebbe raggiungibile da chi ne conosce l'indirizzo |
| Admin | Gestione ruoli e gerarchia. Tocca un utente e cambiagli ruolo davvero: l'elenco si aggiorna. Prova ad aprire **Admin Demo** finché è l'unico amministratore — gli altri due ruoli sono spenti, col motivo scritto. Poi promuovi Leader Demo ad amministratore e riapri: adesso si può. Togliere il ruolo all'ultimo admin lascerebbe l'app senza nessuno che possa assegnarne, e si rimedierebbe solo dal database; nell'app il divieto è nel database (0028), non solo nella schermata |
