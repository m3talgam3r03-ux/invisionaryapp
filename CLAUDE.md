...
- `knowledge/` — corpus versionato in Markdown con front-matter (`title`, `domain`, `tags`); chunking per sezione (`chunkMarkdown`). Vedi `knowledge/README.md`.
- **Il corpus è incluso nell'app**: `npm run corpus` genera `src/lib/corpus.generated.ts` (file generato, versionato, **non modificare a mano**) e l'admin lo carica con un tocco da *Agente AI → Base di conoscenza*. Da riga di comando resta `npm run knowledge` (`--dry`, `--only=<ramo>`).
- `src/lib/domains.ts` duplica i domini per il bundle RN (Deno e React Native non condividono moduli): `npm run eval` fallisce se diverge da `brain.ts` o se il corpus generato è indietro rispetto ai Markdown.
